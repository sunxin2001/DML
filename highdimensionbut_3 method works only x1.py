...
def generate_data(n=2000, theta=1.0, seed=None):
    ...
    g = (0.5 * np.sin(np.pi * X[:, 0] * X[:, 1]) + 
         0.3 * X[:, 2]**2 + 
         0.5 * np.exp(-X[:, 3]**2) + 
         0.2 * np.abs(X[:, 4]))
    

    m = 0.5 * X[:, 0] + 0.3 * np.cos(X[:, 1])
...
def run_high_dim_comparison(R=30, n=15000, theta0=1.0):
    ...
    for r in range(R):
        df = generate_high_dim_plr(n=n, theta=theta0, seed=r+2025)
        ...
        results["DML (All X)"].append(est_dml_high_dim(df, seed=r))
